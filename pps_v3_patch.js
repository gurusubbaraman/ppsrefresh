...
/* ============================================================ */
/* SESSION_0M_LOADED — Sidebar UX Improvements (Nov 2026)       */
/* Fix 1: Count indicator                                        */
/* Fix 2: Scroll-into-view + highlight on marker click           */
/* Fix 3: Scroll-for-more fade indicator                         */
/* ============================================================ */
(function(){
  window.SESSION_0M_LOADED = true;
  console.log("[Session 0M] Sidebar UX patch loading...");
  
  var style = document.createElement("style");
  style.textContent = [
    ".pps-count-bar { position: sticky; top: 0; z-index: 100; background: linear-gradient(135deg, #8B0000 0%, #A0522D 100%); color: #FFD700; padding: 10px 14px; font-family: system-ui, -apple-system, sans-serif; font-size: 13px; font-weight: 600; border-bottom: 2px solid #FFD700; box-shadow: 0 2px 8px rgba(139,0,0,0.3); display: flex; align-items: center; gap: 8px; }",
    ".pps-count-icon { font-size: 16px; }",
    ".pps-count-hint { color: rgba(255,215,0,0.75); font-size: 11px; font-weight: 400; margin-left: auto; }",
    ".pps-card-highlighted { animation: ppsHighlight 2.5s ease-out; background-color: rgba(255,215,0,0.15) !important; }",
    "@keyframes ppsHighlight { 0% { background-color: rgba(255,215,0,0.5); transform: scale(1.02); box-shadow: 0 4px 20px rgba(255,215,0,0.6); } 40% { background-color: rgba(255,215,0,0.35); } 100% { background-color: rgba(255,215,0,0); transform: scale(1); box-shadow: 0 0 0 rgba(255,215,0,0); } }",
    ".pps-scroll-fade { position: sticky; bottom: 0; z-index: 99; height: 40px; background: linear-gradient(180deg, transparent 0%, rgba(255,251,240,0.9) 60%, rgba(255,251,240,1) 100%); pointer-events: none; display: flex; align-items: flex-end; justify-content: center; padding-bottom: 6px; transition: opacity 0.3s ease; }",
    ".pps-scroll-fade.hidden { opacity: 0; }",
    ".pps-scroll-fade-arrow { color: #8B0000; font-size: 14px; font-weight: 700; text-shadow: 0 1px 2px rgba(255,255,255,0.8); animation: ppsBounce 2s ease-in-out infinite; }",
    "@keyframes ppsBounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(3px); } }"
  ].join("\n");
  ...
  function updateCountBar() {
    ...
    var countBar = document.getElementById("pps-count-bar");
    if (!countBar) {
      countBar = document.createElement("div");
      countBar.id = "pps-count-bar";
      countBar.className = "pps-count-bar";
      cardsWrapper.parentNode.insertBefore(countBar, cardsWrapper);
    }
    
    var allCards = cardsWrapper.querySelectorAll(".card");
    var totalCards = allCards.length;
    var visibleCards = 0;
    for (var i = 0; i < allCards.length; i++) {
      var c = allCards[i];
      if (c.style.display !== "none" && !c.hidden) visibleCards++;
    }
    
    var isFiltered = (visibleCards !== totalCards);
    var iconSpan = "<span class='pps-count-icon'>📋</span>";
    var mainText, hintText;
    if (isFiltered) {
      mainText = "Showing <strong>" + visibleCards + "</strong> of <strong>" + totalCards + "</strong> temples";
      hintText = "Clear filters to see all";
    } else {
      mainText = "<strong>" + totalCards + "</strong> temples";
      hintText = "Scroll to explore ↓";
    }
    countBar.innerHTML = iconSpan + "<span>" + mainText + "</span><span class='pps-count-hint'>" + hintText + "</span>";
  }
  
  window.ppsScrollToCard = function(sno) {
    var card = document.querySelector('.card[data-sno="' + sno + '"]');
    if (!card) { console.log("[Session 0M] Card not found for sno " + sno); return; }
    
    var sidebar = document.getElementById("sidebar");
    if (!sidebar) return;
    
    var priorHighlight = sidebar.querySelector(".pps-card-highlighted");
    if (priorHighlight) priorHighlight.classList.remove("pps-card-highlighted");
    
    var offset = card.offsetTop - 60;
    sidebar.scrollTo({ top: offset, behavior: "smooth" });
    
    setTimeout(function() {
      card.classList.add("pps-card-highlighted");
      setTimeout(function() {
        card.classList.remove("pps-card-highlighted");
      }, 2600);
    }, 400);
  };
  
  function hookMapMarkers() {
    if (!window.map || typeof window.map.eachLayer !== "function") {
      setTimeout(hookMapMarkers, 500);
      return;
    }
    
    var hooked = 0;
    window.map.eachLayer(function(layer) {
      if ((layer instanceof L.CircleMarker || layer instanceof L.Marker) && layer._pps0MHooked !== true) {
        ...
        layer.on("click", function() {
          var sno = extractSno();
          if (sno !== null) {
            setTimeout(function() { window.ppsScrollToCard(sno); }, 100);
          }
        });
        layer._pps0MHooked = true;
        hooked++;
      }
    });
    
    if (hooked > 0) console.log("[Session 0M] Hooked " + hooked + " map markers");
  }
  ...
  function updateScrollFade() {
    ...
    var fade = document.getElementById("pps-scroll-fade");
    if (!fade) {
      fade = document.createElement("div");
      fade.id = "pps-scroll-fade";
      fade.className = "pps-scroll-fade";
      fade.innerHTML = "<span class='pps-scroll-fade-arrow'>▼ more below</span>";
      sidebar.appendChild(fade);
    }
    
    var scrolledFromBottom = sidebar.scrollHeight - sidebar.scrollTop - sidebar.clientHeight;
  ...
  function initSession0M() {
    ...
    if (cardsWrapper) {
      ...
      filterObserver.observe(cardsWrapper, {
        attributes: true, subtree: true, attributeFilter: ["style", "hidden", "class"]
      });
    }
    
    var sidebar = document.getElementById("sidebar");
    if (sidebar) sidebar.addEventListener("scroll", updateScrollFade);
    
    console.log("[Session 0M] Sidebar UX patch active");
  }
  
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function() { setTimeout(initSession0M, 500); });
  } else {
    setTimeout(initSession0M, 500);
  }
})();
