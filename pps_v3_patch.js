...
/* ============================================================ */
/* SESSION_1A_LOADED — Framework Infrastructure (Nov 2026)      */
/* Bilingual field schema + card enhancement                    */
/* ============================================================ */
(function(){
  window.SESSION_1A_LOADED = true;
  console.log("[Session 1A] Framework infrastructure loading...");
  
  // Initialize enrichment slots — populated by future sessions
  if (!window.TEMPLE_ENRICHMENT) {
    window.TEMPLE_ENRICHMENT = {};
  }
  
  // Placeholder test data for Chidambaram (sno 3) to verify framework works
  // Full anchor-level composition will replace this in Session 1C
  ...
  style.textContent = [
    ...
    ".pps-enrich-sthala-link { display: inline-block; margin-top: 3px; font-size: 11px; font-weight: 600; color: #A0522D; padding: 2px 6px; border-radius: 4px; background: rgba(212,175,55,0.15); cursor: pointer; text-decoration: none; }",
    ".pps-enrich-sthala-link:hover { background: rgba(212,175,55,0.35); }",
  ...
  function enhanceCard(card) {
    ...
    // Remove prior enhancement (idempotent, allows language refresh)
    var prior = card.querySelector(".pps-enrich");
    ...
    if (html) {
      ...
      enrichDiv.className = "pps-enrich";
      enrichDiv.innerHTML = html;
      
      // Insert before card-links (existing SII/Wiki links row)
      var linksEl = card.querySelector(".card-links");
      if (linksEl) {
        linksEl.parentNode.insertBefore(enrichDiv, linksEl);
      } else {
        card.appendChild(enrichDiv);
      }
    }
    
    // Update town display if town_ta available
    ...
    if (locEl && enrich.town_ta) {
      ...
      if (getCurrentLang() === "ta") {
        parts[0] = enrich.town_ta;
      }
  ...
  function enhanceAllCards() {
    ...
    if (count > 0) console.log("[Session 1A] Enhanced " + count + " cards");
  }
  
  function initSession1A() {
    // Initial pass
    enhanceAllCards();
    ...
    // Watch for card re-renders (filter changes, search, etc.)
    var cardsWrapper = document.getElementById("cards");
    if (cardsWrapper) {
      var enrichObserver = new MutationObserver(function(mutations) {
        var childChange = mutations.some(function(m) { return m.type === "childList"; });
        if (childChange) {
          if (window._pps1A_timer) clearTimeout(window._pps1A_timer);
          window._pps1A_timer = setTimeout(enhanceAllCards, 150);
        }
      });
      enrichObserver.observe(cardsWrapper, { childList: true, subtree: false });
    }
    
    console.log("[Session 1A] Framework infrastructure active.");
    console.log("[Session 1A] TEMPLE_ENRICHMENT slots: " + Object.keys(window.TEMPLE_ENRICHMENT).length);
    console.log("[Session 1A] Test data on sno 3 (Chidambaram) — full anchor content in Session 1C.");
  }
  
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function() { setTimeout(initSession1A, 1200); });
  } else {
    setTimeout(initSession1A, 1200);
  }
})();
